...
- [x] Publish to npm as `dvx`
  - Verify name availability
  - Configure package.json for publishing (files, main, bin)
  - Add prepublishOnly script (build + test)
- [x] `npm i -g dvx` works out of the box
- [x] Post-install message with setup instructions

### Standalone Binary

- [x] Build standalone binary via `bun compile` or `pkg`
  - No Node.js required on target machine
  - Single file output
- [x] Binaries for: Linux x64, macOS arm64, macOS x64, Windows x64
- [x] GitHub Releases with binary artifacts
- [x] Installation script (curl | sh pattern for Linux/macOS)

### Init Wizard

- [x] `dvx init` — guided setup
  - Walk through app registration creation in Entra ID
  - Create application user in Dataverse
  - Assign security role
  - Create auth profile
  - Validate connection
- [x] Output step-by-step instructions for manual setup as fallback
...
- [x] `dvx completion <shell>` command to output completion scripts
...
- [x] HTTP/SSE transport for `dvx mcp` (in addition to stdio)
...
- [x] Init wizard: mock Entra + Dataverse APIs
- [x] Shell completion: output validation
- [x] Binary: smoke test on each platform (CI matrix)
- [x] HTTP transport: connection, tool invocation, streaming
